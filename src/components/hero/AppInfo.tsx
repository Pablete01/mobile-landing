...
const AppInfo = ({ title, description, fullDescription, logo, rating, downloads, version, minimumOS, releaseDate }: AppInfoProps) => {
        ...
        <div className="flex flex-1 flex-col md:flex-row gap-8 items-start text-center md:text-left">
            <div className="flex-shrink-0 self-center md:self-start">
                ...
                <div className="group/icon rounded-2xl border border-gray-200/50 dark:border-white/10 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-white/[0.02] dark:to-white/[0.05] p-7 w-56 h-56 flex items-center justify-center shadow-sm hover:shadow-md transition-all duration-300">
                    <img src={logo} alt={`${title} Icon`} className="h-48 w-48 rounded-2xl object-cover group-hover/icon:scale-105 transition-transform duration-300" />
                </div>
            </div>

            <div className="flex flex-1 flex-col">
                <h1 className="text-4xl font-bold text-heading mb-3">{title}</h1>
    ...
    <div className="mt-4 p-4 text-left bg-white dark:bg-white/[0.02] rounded-xl border border-gray-100 dark:border-white/5 text-sm text-gray-600 dark:text-gray-400">
